...
fileread_kernel(struct file *f, uint64 addr, int n)
{
  ...
  if(f->type == FD_PIPE){
    r = piperead_kernel(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
      return -1;
...
// Write to file f.
// addr is a kernel space address.
int
filewrite_kernel(struct file *f, uint64 addr, int n)
{
  int r, ret = 0;
  ...
  if(f->type == FD_PIPE){
    ret = pipewrite_kernel(f->pipe, addr, n);
  } else if(f->type == FD_DEVICE){
    if(f->major < 0 || f->major >= NDEV || !devsw[f->major].write)
      return -1;
